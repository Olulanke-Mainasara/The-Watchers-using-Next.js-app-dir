  ...
  return (
    <footer style={{ scrollSnapAlign: "start" }} className='w-full h-[70vh] flex flex-col px-10 bg-gray-800 text-white allEM:h-fit allEM:px-8 allT:h-fit allT:px-6'>
        <section className='flex gap-10 allIL:gap-0 basis-4/5 allIL:flex-col'>
            <section className='flex flex-col h-full justify-center basis-1/2 allLM:flex-row allLM:items-center allLM:border-white allIL:border-b'>
            ...
            <section className='border-white border-l allEMT:border-l-0 flex flex-wrap justify-evenly h-full basis-1/2 allEMT:justify-between xtraSmall:gap-8 allEMT:py-8'>
                {footerlinks.map(footerlink => {
                    return (
                        <div key={footerlink.id} className='flex flex-col gap-4 justify-center'>
                            <p className='text-xl'>{footerlink.title}</p>

                            <div className='flex flex-col gap-3 text-slate-300'>
                                <Link href={ "#" }><p>{footerlink.linkOne}</p></Link>
                                <Link href={ "#" }><p>{footerlink.linkTwo}</p></Link>
                                <Link href={ "#" }><p>{footerlink.linkThree}</p></Link>
                                <Link href={ "#" }><p>{footerlink.linkFour}</p></Link>
                            </div>
                        </div>
                    )
                })}
